...
export const useBooksStore = defineStore('books', () => {
  ...
  async function fetchList() {
    ...
    error.value = ''
    try {
      items.value = await request('/books')
  ...
  async function create(values) {
    ...
    try {
      ...
      const item = await request('/books', {
        method: 'POST',
  ...
  async function update(id, values) {
    ...
    error.value = ''
    try {
      const current = await request(`/books/${encodeURIComponent(id)}`)
      const item = await request(`/books/${encodeURIComponent(id)}`, {
        method: 'PUT',
